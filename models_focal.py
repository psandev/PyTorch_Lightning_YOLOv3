...
def create_modules(module_defs, apply_focal_loss):
    """
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            yolo_layer = YOLOLayer(anchors, num_classes, apply_focal_loss,img_size)
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, apply_focal_loss, img_dim=416):
    ...
    def compute_bce_loss(self, inputs, targets, apply_focal_loss):
        if apply_focal_loss:
            ...
            # return self.bce_loss(inputs, targets).mean()
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if targets is None:
            ...
            loss_conf_noobj = self.compute_bce_loss(pred_conf[noobj_mask], tconf[noobj_mask],self.apply_focal_loss)
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, apply_focal_loss, img_size=416):
        super(Darknet, self).__init__()
        self.apply_focal_loss = apply_focal_loss
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_modules(self.module_defs,self.apply_focal_loss)
